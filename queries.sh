...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games ON teams.team_id = games.winner_id WHERE teams.team_id=games.winner_id ORDER BY name")"
 
echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year, name FROM teams FULL JOIN games ON teams.team_id = games.winner_id WHERE teams.team_id = games.winner_id AND round = 'Final' ORDER BY year")"
